    ...
    public class BotCredentials : IBotCredentials
    {
        ...
        public BotCredentials()
        {
            ...
            if(!File.Exists(_credsFileName))
                _log.Warn($"credentials.json is missing. Attempting to load creds from environment variables prefixed with 'Leto2Bot_'. Example is in {Path.GetFullPath("./credentials_example.json")}");
            try
            {
                var configBuilder = new ConfigurationBuilder();
                configBuilder.AddJsonFile(_credsFileName, true)
                    .AddEnvironmentVariables("Leto2Bot_");
                ...
                if (string.IsNullOrWhiteSpace(portStr))
                    ShardRunPort = new Leto2Random().Next(5000, 6000);
                else
