...
namespace Leto2bot.Services.Impl
{
    public class SoundCloudApiService
    {
        ...
        public async Task<SoundCloudVideo> ResolveVideoAsync(string url)
        {
            ...
            {
                response = await http.GetStringAsync($"https://scapi.leto2bot.me/resolve?url={url}").ConfigureAwait(false);
            }
        ...
        public async Task<SoundCloudVideo> GetVideoByQueryAsync(string query)
        {
            ...
            using (var http = new HttpClient())
            {
                response = await http.GetStringAsync($"https://scapi.leto2bot.me/tracks?q={Uri.EscapeDataString(query)}").ConfigureAwait(false);
            }
    ...
    public class SoundCloudVideo
    {
        ...
        public async Task<string> StreamLink()
        {
            using (var http = new HttpClient())
            {
                return await http.GetStringAsync($"http://scapi.leto2bot.me/stream/{Id}");
            }
        }
    }
