...
using Leto2bot.DataStructures.Replacements;
...
namespace Leto2bot.Services.CustomReactions
{
    public static class Extensions
    {
        private static readonly Regex imgRegex = new Regex("%(img|image):(?<tag>.*?)%", RegexOptions.Compiled);

        private static readonly Leto2Random rng = new Leto2Random();
        ...
        public static Dictionary<Regex, Func<Match, Task<string>>> regexPlaceholders = new Dictionary<Regex, Func<Match, Task<string>>>()
        {
            { imgRegex, async (match) => {
                ...
                var img = (elems.ElementAtOrDefault(new Leto2Random().Next(0, elems.Length))?.Children?.FirstOrDefault() as IHtmlImageElement);
