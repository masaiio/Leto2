...
namespace Leto2bot.Services.CustomReactions
{
    public class CustomReactionsService : IEarlyBlockingExecutor
    {
        ...
        public CustomReactionsService(PermissionService perms, DbService db, LetoStrings strings,
        ...
        public CustomReaction TryGetCustomReaction(IUserMessage umsg)
        {
            ...
            if (GuildReactions.TryGetValue(channel.Guild.Id, out CustomReaction[] reactions))
                if (reactions != null && reactions.Any())
                {
                    ...
                    if (rs.Length != 0)
                    {
                        var reaction = rs[new Leto2Random().Next(0, rs.Length)];
                        if (reaction != null)
                        {
            ...
            if (grs.Length == 0)
                return null;
            var greaction = grs[new Leto2Random().Next(0, grs.Length)];
