...
using Leto2bot.Services.Music.SongResolver;
using Leto2bot.Services.Impl;

namespace Leto2bot.Services.Music
{
    public class MusicService
    {
        ...
        public MusicService(DiscordSocketClient client, IGoogleApiService google,
            LetoStrings strings, ILocalization localization, DbService db,
        ...
        public async Task TryQueueRelatedSongAsync(SongInfo song, ITextChannel txtCh, IVoiceChannel vch)
        {
            ...
            var si = await ResolveSong(related[new Leto2Random().Next(related.Length)], _client.CurrentUser.ToString(), MusicType.YouTube);
            if (si == null)
