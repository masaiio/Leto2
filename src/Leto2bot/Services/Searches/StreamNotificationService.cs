...
namespace Leto2bot.Services.Searches
{
    public class StreamNotificationService
    {
        ...
        public StreamNotificationService(DbService db, DiscordSocketClient client, LetoStrings strings)
        {
            _db = db;
            _client = client;
            _strings = strings;
        ...
        public EmbedBuilder GetEmbed(FollowedStream fs, StreamStatus status, ulong guildId)
        {
            var embed = new EmbedBuilder().WithTitle(fs.Username)
                                          ...
                                          .AddField(efb => efb.WithName(GetText(fs, "platform"))
                                                            ...
                                                            .WithIsInline(true))
                                          .WithColor(status.IsLive ? Leto2bot.OkColor : Leto2bot.ErrorColor);
