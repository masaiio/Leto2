...
namespace Leto2bot.Modules.Permissions
{
    ...
    {
        [Group]
        public class CommandCostCommands : Leto2Submodule
        {
            ...
            {
                //_commandCosts = new ConcurrentDictionary<string, int>(Leto2Bot.BotConfig.CommandCosts.ToDictionary(
            ...
            //    else
            //        await Context.Channel.SendConfirmAsync($"{Format.Bold(cmd.Name)} now costs {cost}{Leto2Bot.BotConfig.CurrencySign} to run.").ConfigureAwait(false);
            //}
        }
    }
}
